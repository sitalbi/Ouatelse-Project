...
public class AuthStoreController extends BaseController {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        this.entityManagerStore.getQueryForAll().forEach(store -> storeComboBox.getItems().add(store));
    ...
    public void onConnectionButtonClick() {
        if (storeComboBox.getSelectionModel().isEmpty() || passwordField.getText().trim().isEmpty()) {
            this.errorMessageField.setText("Veuillez remplir tout les champs");
            return;
        }
        Store store = this.entityManagerStore.authGetStoreIfExists(
                storeComboBox.getSelectionModel().getSelectedItem().getId(),
